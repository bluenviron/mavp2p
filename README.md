...
Flags:
  ...
  -q, --quiet                                   Suppress info messages.
      ...
      --streamreq-disable                       Do not request streams to Ardupilot devices, that need an explicit request in order to emit telemetry streams. This task is usually delegated to the router, in order
                                                to avoid conflicts when multiple ground stations are active.
      --streamreq-frequency=4                   Stream frequency to request.
      --dump                                    Dump telemetry to disk
      --dump-folder="dump/"                     Dump folder
      --dump-file="2006-01-02_15-04-05.tlog"    Dump file, in time.Format() format
```
